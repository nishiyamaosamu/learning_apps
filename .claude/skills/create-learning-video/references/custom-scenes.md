...
**カード・帯の中のテキストに `whiteSpace: "nowrap"` を付けない。** 折り返しを止めると、
入り切らない文字はカードの角丸の外へそのまま流れ出す（clip されない）。カードの地は
`colors.surface`（白）でページ地も `colors.bg`（ほぼ白）なので、**あふれた文字は背景と
同化して「少し長いだけ」に見え、静止画レビューを通り抜ける**（sg-L36 s10 の検疫ネットワークの
3本の帯で発生。人のレビューで差し戻された）。幅の決まった入れ物に文を入れるときは
nowrap を使わず、文字側に `minWidth: 0` を付けて自然な折り返しを逃げ道に残す。
そのうえで stills を見るときは **1行ずつ、文字の右端がカードの角丸の内側で終わっているか**を
確認する（帯の下端・字幕帯との重なりだけ見ていると見落とす）。
横並び（テキスト列 + イラスト）で入らないときは、文を削る前に `flex` の比を
テキスト側へ寄せる（例 1.6/0.85 → 1.95/0.7）+ 文字を 1〜1.5×SCALE 落とすので足りることが多い。
