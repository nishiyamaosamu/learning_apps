...
| prop-ledger ✅ | 情報資産台帳（表組みの帳簿）。**絵の中に「情報資産台帳」と描かれている**ので、他の帳簿・書類の文脈に流用しないこと（sg-L54 で経理の書類として使い差し戻し） | L79, L80 |
...
| prop-accounting-papers ✅ | 経理の紙書類（請求書・領収書）が積み重なった束。押印つき。**prop-ledger（情報資産台帳のバインダー）とは別物**で、経理・保存の文脈はこちらを使う | L54 |
