 ...
 * videoId に "common" を渡すと public/audio/common/*.mp3（全動画共通の使い回し音声）を
 * src/videos/common/common.audio.json に書き出す（動画別サブディレクトリを経由しない）。
 *
 *   node scripts/audio-durations.mjs <videoId>
 *   node scripts/audio-durations.mjs common
 *
...
const isCommon = videoId === "common";
const appDir = isCommon
  ? "common"
  : videoId.startsWith("ip-")
    ? "ipa_ip"
    : videoId.startsWith("sg-")
      ? "ipa_sg"
      : "demo";

const dir = isCommon ? join(root, "public", "audio", "common") : join(root, "public", "audio", appDir, videoId);
if (!existsSync(dir)) {
  console.error(
    isCommon
      ? "public/audio/common/ がありません"
      : `public/audio/${appDir}/${videoId}/ がありません。先に tts.py で音声を生成してください`,
  );
  process.exit(1);
}
const files = readdirSync(dir)
  ...
  .sort();
if (files.length === 0) {
  console.error(`${dir} に mp3 がありません`);
  process.exit(1);
}
...
const outRelPath = isCommon ? join("src", "videos", "common", "common.audio.json") : join("src", "videos", appDir, `${videoId}.audio.json`);
const outPath = join(root, outRelPath);
writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n");

console.log(`${outRelPath} に ${files.length} ファイル分を書き出しました`);
