 ...
 *   ISMS → アイエスエムエス
 *   JIS Q 27001 → ジス キュー にまんななせんいち（27002 → …にまんななせんに /
 *   27014 → …にまんななせんじゅうよん）
 *   （v1 は数字のまま渡して差し戻し。v2 は数字だけ仮名にして「JIS Q」をラテン文字で残したため
 *   継ぎ目が不自然でさらに差し戻し。v3 で L28 s03-4 の確定形に合わせ、規格名は記号部まで仮名、
 *   かつ**仮名の数字の後ろに必ず半角スペース**を入れた。字幕・画面は数字表記のまま。
 *   s03-1・s03-5・s04-1・s11-3 は数字の直後に「です」「は」「に」が地続きで不自然だったため、
 *   字幕ごと言い回しを変えて数字の後ろに「という規格」「が」を置いた）
 */
...
const SEG_STD = [
  N("s03-1.mp3", "ISMSの要求事項は、JIS Q 27001という規格に定められています。"),
  N("s03-2.mp3", "こちらは、何を満たすべきかを示す規格です。"),
  N("s03-3.mp3", "対になるのが、JIS Q 27002という規格です。"),
  N("s03-4.mp3", "こちらは、具体的にどうやるかをまとめた、実践の手引です。"),
  N("s03-5.mp3", "27001が認証の基準、27002が対策を選ぶためのカタログです。"),
];

const SEG_CONTROL = [
  N("s04-1.mp3", "JIS Q 27002という規格に並ぶ対策を、管理策といいます。"),
...
const SEG_Q1 = [
  ...
  N("s11-3.mp3", "正解は、JIS Q 27002という規格です。", { gapBeforeSec: 1.8 }),
];
